...
import com.szchmtech.erefreshlib.library.internal.FlipLoadingLayout;
import com.szchmtech.erefreshlib.library.internal.LoadingLayout;
import com.szchmtech.erefreshlib.library.internal.RotateLoadingLayout;
import com.szchmtech.erefreshlib.library.internal.Utils;
import com.szchmtech.erefreshlib.library.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    protected float mInitialMotionRawY;
    ...
    private boolean mShowRefreshSuccess = false;
...
//                    Log.e("aaa1", "mInitialMotionY: " + mInitialMotionY + " , mInitialMotionRawY" + mInitialMotionRawY);
                    mIsBeingDragged = false;
                }
    ...
    @Override
    public final void onRefreshComplete(boolean isSuccess) {
        if (isRefreshing()) {
            if (mShowRefreshSuccess && isSuccess) {
                onSuccess();
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_DOWN: {
                if (isReadyForPull()) {
                    mLastMotionY = mInitialMotionY = event.getY();
                    mLastMotionX = mInitialMotionX = event.getX();
//                    Log.e("aaa2", "mInitialMotionY: " + mInitialMotionY + " , mInitialMotionRawY" + mInitialMotionRawY);
                    mIsBeingDragged = true;
                    return true;
                }
                mInitialMotionRawY = event.getRawY();
    ...
    protected void onSuccess() {
        mHeaderLayout.setSuccess();
        mFooterLayout.setSuccess();
    }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (a.hasValue(R.styleable.PullToRefresh_ptrRefreshSuccess)) {
            mShowRefreshSuccess = a.getBoolean(R.styleable.PullToRefresh_ptrRefreshSuccess, false);
        }
