...
//		BigDecimal bb = new BigDecimal(d);
	 ...
	 * 根据手机的分辨率从 dp 的单位 转成为 px(像素)
	 */
	public static int setSp2px(Context context, float dpValue) {
	...
	public static String deleterTrim(String edit) {
		...
		for (int i = 0, len = edit.length(); i < len; i++) {
			if (!edit.substring(i, i + 1).equals(" ")) {
				Str = Str.concat(edit.substring(i, i + 1));
			}
		}
	...
	/**
	 * 方法描述：  设置edittext的	hint 大小
	 * @param           size 需要设置的hint size
	 * @return
	 */
	public static void setHintSize(Context context, EditText editText,float size) {
		// 新建一个可以添加属性的文本对象
		SpannableString ss = new SpannableString(editText.getHint().toString());
		// 新建一个属性对象,设置文字的大小
		AbsoluteSizeSpan ass = new AbsoluteSizeSpan(setSp2px(context,size), false);
		// 附加属性到文本
	...
	public static void resetFragmentView(Activity activity, Fragment fragment) {
	 ...
	 * @return
	 */
	public static boolean isSmallerKitCat() {
